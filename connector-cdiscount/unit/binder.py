...
class CdiscountBinder(Binder):
    """  Binder for cdiscount  """


@cdiscount
class CdiscountModelBinder(CdiscountBinder):
    """
    Bindings are done directly on the binding model.

    Binding models are models called ``cdiscount.{normal_model}``,
    like ``cdiscount.res.partner`` or ``cdiscount.product.product``.
    They are ``_inherits`` of the normal models and contains
    the Cdiscount ID, the ID of the Cdiscount Backend and the additional
    fields belonging to the Cdiscount instance.
    """
    _model_name = [
    ...
    def to_backend(self, record_id, wrap=False):
        ...
        if wrap:
            ...
            if binding:
                binding.ensure_one()
                return binding.cdiscount_id
            else:
                return None
        if not record:
            record = self.model.browse(record_id)
        assert record
        return record.cdiscount_id
    ...
    def bind(self, external_id, binding_id):
        ...
        :type binding_id: int
        """
        # the external ID can be 0 on Cdiscount! Prevent False values
        ...
        # avoid to trigger the export when we modify the `cdiscount_id`
    ...
    def unwrap_binding(self, binding_id, browse=False):
        ...
        Example: when called with a ``cdiscount.product.product`` id,
    ...
    def unwrap_model(self):
        ...
        Example: when called on a binder for ``cdiscount.product.product``,
